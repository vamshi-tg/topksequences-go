...
var (
	...
	sequenceCountMap SequenceCountMap             // global map to store the frequency of 'n' word sequences from all the given files/text.
)
...
*/
func Execute() {
	sequenceCountMap = NewSequenceCountMap()
	...
	<-mergingMapsDone
	topKSequences := getTopKSequences(sequenceCountMap, k)
...
/*
	processText scans the text, builds the sequenceCountMap and pushes it to a channel.

	Params:
	r - source of the text to be processed
	sequenceCountMapStream - channel to which the resultant sequenceCountMap is pushed

	Result:
	sequenceCountMap - Holds the count of all the 'n' word sequences present int the text
...
func processText(r io.Reader, sequenceCountMapStream chan<- SequenceCountMap) {
	...
	_sequenceCountMap := NewSequenceCountMap()

	// abort when enough text is not present
	if windowDeque.Len() < n {
		sequenceCountMapStream <- _sequenceCountMap
		return
	}
	...
	for scanner.Scan() {
		...
		_sequenceCountMap.IncCount(key)
...
/*
	getTopKSequences returns the top k n-word sequences from the global sequenceCountMap.
...
/*
    ...
    merges it into the global sequenceCountMap.
...
func mergeSequenceCountMaps(sequenceCountMapStream <-chan SequenceCountMap, done chan<- bool) {
	for val := range sequenceCountMapStream {
		sequenceCountMap = mergeMaps(sequenceCountMap, val)
	}
	done <- true
}
